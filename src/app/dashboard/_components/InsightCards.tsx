...
export function InsightCards({ insights, loading = false }: InsightCardsProps) {
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-muted-foreground">إنجازات وتنبيهات</span>
      </div>
      ...
      {loading && insights.length === 0 ? (
        ...
        <p className="text-xs text-muted-foreground">
          لسه ما فيش رؤى. ابدأ بمحاكاة عملية شراء وهتشوف إنجازاتك هنا.
        </p>
      ) : (
        <div className="flex gap-4 overflow-x-auto pb-3 -mx-1 px-1 snap-x snap-mandatory scroll-smooth">
          {insights.slice(0, 5).map((insight, i) => (
            ...
            >
              <GlassCard
                className={`p-4 h-full border transition-colors duration-500 ${isAiActive ? "border-purple-400/50 bg-purple-50/5" : "border-border/60"}`}
              >
                ...
                {insight.highlight && (
                  <div
                    className={`inline-block text-xs font-bold px-2.5 py-1 rounded-full transition-colors duration-500 ${isAiActive ? "bg-purple-500 text-white" : "bg-sukuk-green-muted text-sukuk-green"}`}
                  >
                    {insight.highlight}
                  </div>
                )}
